...
object CriterialRef {
  ...
  private[criterial] def nothing[T <: CriterialTag]: CriterialValue[Nothing, T] =
    new CriterialValue[Nothing, T] {
      override def ref: Criterial[T] = Criterial.pure("")
    }
  private[criterial] def value[V, D <: CriterialTag](v: V): CriterialValue[V, D] =
    new CriterialValue[V, D] {
  ...
  private[criterial] def bool[D <: CriterialTag](v: Boolean): CriterialBool[D] =
    new CriterialBool[D] {
      override def ref: Criterial[D] = Criterial.pure(v.toString)
    }
