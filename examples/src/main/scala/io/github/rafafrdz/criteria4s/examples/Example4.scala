...
object Example4 extends App {
  ...
  def expr[T <: CriteriaTag: LEQ: EQ: AND: OR: ({type A[D <: CriteriaTag] = Show[Column, D]})#A]: Criteria[T] =
    (col[T]("a") leq lit(3)) and
      (col[T]("b") leq lit(4)) or
      (col[T]("c") === lit("c"))

  def inlineExpr[T <: CriteriaTag: LEQ: EQ: AND: OR: ({type A[D <: CriteriaTag] = Show[Column, D]})#A]: Criteria[T] =
