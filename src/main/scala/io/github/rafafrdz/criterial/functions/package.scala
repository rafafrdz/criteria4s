...
package object functions extends predicates with conjunctions {
  ...
  def pred[T <: CriterialTag, H <: CriterialPredOp[T]](cr1: CriterialRef[T], cr2: CriterialRef[T])(implicit
      H: H
  ): Criterial[T] =
    H.eval(cr1, cr2)

  def cond[T <: CriterialTag, H <: CriterialCondOp[T]](cr1: Criterial[T], cr2: Criterial[T])(implicit
      H: H
  ): Criterial[T] =
    H.eval(cr1, cr2)
