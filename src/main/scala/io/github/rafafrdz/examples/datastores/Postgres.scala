...
object Postgres {
  ...
  implicit val isnotnullPred: ISNOTNULL[Postgres] = build1[Postgres, ISNOTNULL](opeExpr1("IS NOT NULL"))

  implicit val betweenPred: BETWEEN[Postgres] = build[Postgres, BETWEEN](opeExpr("BETWEEN"))

  implicit val notbetweenPred: NOTBETWEEN[Postgres] = build[Postgres, NOTBETWEEN](opeExpr("NOT BETWEEN"))
